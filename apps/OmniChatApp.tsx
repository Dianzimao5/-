...
import { Settings, Trash2, ArrowLeft, RefreshCw, Plus, Image as ImageIcon, XCircle, Scroll, MessageCircle, MoreHorizontal, Send, Users, User, Hash, Edit3, LogOut, Shield, UserPlus } from 'lucide-react';
...
// Sub-components to fix Hook Rules (Error #310)
...
const EditContact = ({ theme, langText, popRoute, params, contacts, setContacts, config, setConfig, handleCreateContact, handleDeleteContact }: any) => {
    ...
    // For 'isMe', init with userProfile. For NPCs, init with existing contact or empty.
    const [form, setForm] = useState(exist || (isMe ? { ...config.userProfile, bio: config.userProfile.likes } : {}) as any);

    const save = () => {
          if (isMe) {
              // Update Global Profile via setConfig
              setConfig((prev: AppConfig) => ({
                  ...
                  userProfile: {
                      ...
                      likes: form.bio // Mapping bio back to likes as per data structure
                  }
              }));
              popRoute();
          } else if (isNew) {
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
                <div className="flex justify-center mb-4">
                    <img src={form.avatar} className="w-24 h-24 rounded-full bg-gray-500 object-cover" />
                </div>
                ...
                <div className="space-y-1">
                    <label className="text-xs opacity-50 font-bold">{langText.oc_label_uid}</label>
                    <input value={form.uid||''} onChange={e=>setForm({...form, uid:e.target.value})} placeholder={isMe ? config.userProfile.uid : 'Auto'} className={`w-full p-3 rounded-lg border bg-transparent ${theme.id==='night'?'border-cyan-900':'border-gray-300'}`}/>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                    <label className="text-xs opacity-50 font-bold">Background URL</label>
                    <input value={form.bgImage||''} onChange={e=>setForm({...form, bgImage:e.target.value})} className={`w-full p-3 rounded-lg border bg-transparent ${theme.id==='night'?'border-cyan-900':'border-gray-300'}`}/>
                </div>
        ...
        <div className="h-full flex flex-col bg-neutral-50/5">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div className="text-center">
                    <img src={g.avatar} className="w-20 h-20 rounded-full mx-auto mb-2 bg-gray-200" />
                            ...
                            return (
                                <div key={mid} className="relative group">
                                    <div onClick={() => isTransferring && updateGroup('ownerId', mid)} className="flex flex-col items-center gap-1 cursor-pointer">
                                        <img src={m.avatar} className={`w-10 h-10 rounded-full object-cover ${g.ownerId === mid ? 'border-2 border-yellow-500' : ''}`} />
                                        <span className="text-[9px] truncate w-full text-center">{m.name}</span>
                                    </div>
                                    {/* Remove Button for Owner (except self) */}
    ...
    return (
        <div className="h-full flex flex-col bg-neutral-900/5">
            ...
            <div className={`flex-1 -mt-6 rounded-t-3xl relative px-6 pt-16 overflow-y-auto ${theme.windowBg}`}>
                <img src={c.avatar} className={`absolute -top-12 left-6 w-24 h-24 rounded-full border-4 ${theme.id==='night'?'border-gray-900':'border-white'}`} alt="Av"/>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        <h3 className="text-xs font-bold opacity-50 uppercase mb-2">Traits</h3>
                        <div className="flex flex-wrap gap-2">
                            {c.tags?.map((t: string) => <span key={t} className={`text-xs px-2 py-1 rounded-full border opacity-70`}>{t}</span>)}
                        </div>
                    </div>
                </div>
            </div>
...
const OmniChatApp: React.FC<Props> = ({ config, setConfig, assistant, theme, currentWorld, langText }) => {
  ...
  const getTargetName = (id: string) => getContact(id)?.name || getGroup(id)?.name || 'Unknown';
  ...
  useEffect(() => {
    if (currentRoute.name === 'chat_room' && !isTyping) messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [chats, currentRoute, isTyping]);
  ...
  useEffect(() => {
    if (isTyping && streamingContent.length < fullResponse.length) {
      ...
      setIsTyping(false);
      const targetId = currentRoute.params.id;
      const sender = currentRoute.params.isGroup ? getContact(groups.find(g=>g.id===targetId)?.members[0] || '') : getContact(targetId);
  ...
  const handleSendMessage = async (textOverride: string | null = null) => {
    ...
    const targetId = currentRoute.params.id;
    const isGroup = currentRoute.params.isGroup;

    if (!config.apiKey) {
         alert(langText.sys_tip_key);
         return;
    }
    ...
    try {
        ...
        if (isGroup) {
            ...
            const memberNames = group?.members.map(mid => getContact(mid)?.name).join(', ');
        ...
        if (config.provider === 'gemini') {
             ...
             const res = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    contents: apiMessages.filter(m=>m.role!=='system').map(m => ({ role: m.role==='user'?'user':'model', parts: [{ text: m.content || ' ' }] }))
                })
             });
                     ...
                     return (
                         <div key={id} onClick={() => pushRoute('chat_room', { id, isGroup })} className={`flex items-center gap-3 p-3 mb-1 rounded-xl cursor-pointer ${theme.id==='night'?'hover:bg-cyan-900/20':'hover:bg-white/60'}`}>
                             <div className="relative">
                                <img src={target.avatar} className={`w-12 h-12 rounded-full object-cover border ${theme.id==='night'?'border-cyan-800':'border-gray-200'}`} alt="Av" />
      ...
      <div className="flex-1 overflow-y-auto p-4">
          ...
          {groups.map(g => (
              <div key={g.id} onClick={() => pushRoute('chat_room', { id: g.id, isGroup: true })} className={`flex items-center gap-3 p-3 mb-2 rounded-xl cursor-pointer ${theme.id==='night'?'bg-cyan-900/10':'bg-white/40'}`}>
                  <img src={g.avatar} className="w-10 h-10 rounded-full" alt="G"/>
          ...
          {contacts.map(c => (
              <div key={c.id} onClick={() => pushRoute('profile_view', { id: c.id })} className={`flex items-center gap-3 p-3 mb-2 rounded-xl cursor-pointer ${theme.id==='night'?'bg-cyan-900/10':'bg-white/40'}`}>
                  <img src={c.avatar} className="w-10 h-10 rounded-full object-cover" alt="C"/>
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-6 -mt-10 relative mb-4">
                  <div className="flex justify-between items-end">
                      <img src={p.avatar || 'https://api.dicebear.com/7.x/adventurer/svg?seed=User'} className={`w-24 h-24 rounded-full border-4 ${theme.id==='night'?'border-black':'border-white'}`} alt="Me"/>
              ...
              <div className="px-6 space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                      <div className={`p-3 rounded-xl text-center border ${theme.id==='night'?'bg-cyan-900/20 border-cyan-800':'bg-white border-gray-200'}`}>
                          <div className="text-xl font-bold">{p.level || 5}</div>
                          <div className="text-[10px] opacity-60">{langText.oc_label_level}</div>
                      </div>
                      <div className={`p-3 rounded-xl text-center border ${theme.id==='night'?'bg-cyan-900/20 border-cyan-800':'bg-white border-gray-200'}`}>
                          <div className="text-xl font-bold">{contacts.length}</div>
                          <div className="text-[10px] opacity-60">{langText.oc_tab_contact}</div>
                      </div>
                      <div className={`p-3 rounded-xl text-center border ${theme.id==='night'?'bg-cyan-900/20 border-cyan-800':'bg-white border-gray-200'}`}>
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {msgs.map((msg) => (
                    <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-end`}>
                        {msg.role !== 'user' && (
                            <img src={isGroup ? getTargetAvatar(msg.senderId || '') : target.avatar} className="w-8 h-8 rounded-full border border-black/5" />
                        )}
            ...
            <div className={`p-3 border-t ${theme.id==='night'?'bg-gray-900 border-cyan-900':'bg-white border-gray-100'}`}>
                <div className="flex gap-2">
                    <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSendMessage()} placeholder={langText.msg_placeholder} className={`flex-1 px-4 py-2 rounded-full outline-none text-sm ${theme.inputBg}`}/>
                    <button onClick={() => handleSendMessage()} disabled={!input} className={`p-2 rounded-full ${theme.id==='simple'?'bg-[#06c755] text-white':'bg-blue-600 text-white'}`}><Send size={20}/></button>
                </div>
            </div>
        </div>
    );
  ...
  if (currentRoute.name === 'chat_room') return renderChatRoom();
