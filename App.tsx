...
import { Bot, BookOpen, Settings, LayoutGrid, Home, Signal, Wifi, Battery, Zap, Sparkles, MessageSquare } from 'lucide-react';
...
const AppIcon = ({ icon: Icon, label, onClick, color, theme, size = 'md' }: any) => (
  <button 
    onClick={onClick}
    className="flex flex-col items-center gap-2 group transition-transform hover:scale-105 active:scale-95"
  >
    <div className={`${size === 'lg' ? 'w-16 h-16 md:w-20 md:h-20' : 'w-14 h-14 md:w-16 md:h-16'} flex items-center justify-center text-white shadow-lg ${theme.appIconShape} ${color}`}>
...
export default function App() {
  ...
  const renderApp = () => {
    switch(activeApp) {
        case 'chat': return <OmniChatApp config={config} setConfig={setConfig} assistant={assistant} setAssistant={setAssistant} theme={currentTheme} currentWorld={currentWorld} langText={langText}/>;
        case 'assistant': return <AssistantApp config={config} assistant={assistant} setAssistant={setAssistant} savedWorlds={savedWorlds} theme={currentTheme} currentWorld={currentWorld} langText={langText}/>;
      ...
      <div className={`relative transition-all duration-500 ease-in-out w-full max-w-[960px] aspect-[9/16] md:aspect-[16/10] ${currentTheme.deviceBorder} rounded-[32px] md:rounded-[48px] shadow-2xl flex flex-col overflow-hidden ring-4 ring-black/10 ${currentTheme.font}`}>
        ...
        <div className={`flex-1 relative overflow-hidden flex flex-col ${currentTheme.wallpaper} bg-cover bg-center transition-all duration-500`}>
          ...
          <div className="flex-1 relative p-6 overflow-hidden flex flex-col">
            ...
            {/* App Grid */}
            <div className={`flex-1 grid grid-cols-4 grid-rows-4 gap-6 p-4 content-start transition-all duration-500 ${activeApp ? 'scale-110 opacity-0 pointer-events-none filter blur-sm' : 'scale-100 opacity-100'}`}>
                <AppIcon icon={Bot} label={langText.app_assistant} theme={currentTheme} color="bg-indigo-600" onClick={() => setActiveApp('assistant')} />
                <AppIcon icon={MessageSquare} label={langText.app_chat} theme={currentTheme} color="bg-green-500" onClick={() => setActiveApp('chat')} />
            ...
            {/* Active App View - Absolute positioning on top of grid */}
            <div className={`absolute inset-0 md:inset-8 transition-all duration-300 transform ${activeApp ? 'translate-y-0 opacity-100' : 'translate-y-[100%] opacity-0 pointer-events-none'} ${currentTheme.windowBg} md:rounded-3xl shadow-2xl flex flex-col overflow-hidden z-30`}>
                {activeApp && renderApp()}
            </div>
          ...
          {/* Home Bar */}
          <div className="h-10 w-full flex items-center justify-center z-30 pb-2">
